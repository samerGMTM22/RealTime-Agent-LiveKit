...
async def entrypoint(ctx: JobContext):
    ...
    logger.info(f"System prompt: {config.get('system_prompt', 'Default')[:100]}...")
    logger.info(f"Voice: {config.get('voice_model', 'echo')}, Temperature: {config.get('temperature', 102)/100}")
    ...
    try:
        ...
        voice = config.get("voice_model", "echo")
        realtime_temp = max(0.6, min(1.2, config.get("temperature", 102) / 100))
        
        # Create assistant with tools defined at module level
        ...
        # Add function tools to the session
        session.llm.add_function(search_web)
        session.llm.add_function(send_email)
        
        logger.info("Function tools added to session")
        ...
        session = AgentSession(
            vad=silero.VAD.load(),
            stt=openai.STT(),
            llm=openai.LLM(model="gpt-4o", instructions=config.get("system_prompt")),
            tts=openai.TTS(),
        )
        
        # Add tools to standard pipeline
